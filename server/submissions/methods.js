    ...
    'submissions.edit'(data) {
        ...
        if (results.length > 0) {
            ...
            _.each(results, (c) => {
                ...
                let resultUserID = c.userID
                // check if result affected is virtual race
                if(c.race_type == 'virtual_race') {
                    // If the submission is for partner run
                    if (Array.isArray(resultUserID)){
                        let submitUserArray = c.submitBy;
                        if(submitUserArray.length === 1){
                            // if only one submission is present
                            newResultDistanceFix = newDistanceFix;
                            newResultTiming = timingInSec;
                            new_timing_per_km = Math.round(parseInt(newResultTiming) / newResultDistanceFix);
                        }else{
                            // if submission is two
                            if (newResultDistance < parseInt(c.category)){
                                AllResults.update(
                                    {_id:  c._id},
                                    { $pull: {
                                        submitBy: oldSubmission.userID,
                                        submissions: id,
                                    }});
                                newResultDistanceFix = oldResultDistance - oldDistance;
                                newResultTiming = oldResultTiming - oldTimingInSec;
                                new_timing_per_km = Math.round(parseInt(newResultTiming) / newResultDistance);
                                messageToUser = "Submission will not be taken in account for Partner Run";
                            }else{
                                new_timing_per_km = Math.round(parseInt(newResultTiming) / newResultDistanceFix);
                            }
                        ...
                        // Normal Virtual race without any partner

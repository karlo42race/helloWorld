...
const createResultWithOrder = (oneOrder, partnerRun=false) => {
    ...
    let resultExists, values;
    if (partnerRun){
        let partnerOrder = Orders.findOne({$and: [{orderNum: oneOrder.orderNum}, {userID: {$ne: userID} } ]});
        let partnerUser = Meteor.users.findOne({_id: partnerOrder.userID});
        resultExists = AllResults.findOne({$and: [{userID: {$in: [userID,]}}, {raceID: raceID}] });
        values = {
            category,
            team,
            userID,
            email,
            partnerSelected: partnerUser,

        };
        sendEmailWithOrderForPartnerRace(oneOrder, values, oneRace, currentUser, partnerUser);
    }else{
        resultExists = AllResults.findOne({$and: [{userID: userID}, {raceID: raceID}] });
        values = {
            category,
            team,
            userID,
...
const sendEmailWithOrderForPartnerRace = (oneOrder, values, raceData, currentUser, partnerData) => {
    let orderTimestamp = formatDate(new Date());
    Email.send({
        to: 'sales@42race.com',
        from: "42Race <contact@42race.com>",
        subject: "[42Race] New runner joins" +raceData.race_name,
        html: OrderEmailToAdmin(currentUser, oneOrder, raceData, orderTimestamp, oneOrder.orderNum),
    });
    ...
    Email.send({
        ...
        html: OrderEmailToUserWithBuddy(currentUser, oneOrder, raceData, orderTimestamp, oneOrder.orderNum, partnerData),
    });
    console.log("Orders: Email sent to user");
    // to partner
    Email.send({
        to: partnerData.emails[0].address,
        from: "42Race <contact@42race.com>",
        subject: "[42Race] You have accepted " + raceData.race_name,
        html: OrderEmailToBuddy(currentUser, oneOrder, raceData, orderTimestamp, oneOrder.orderNum, partnerData),
    });
    console.log("Orders: Email sent to buddy");
}
...
Meteor.methods({
    'xfers.charge'(raceData, values, userData) {
        ...
        try {
            const result = HTTP.call("POST", `${process.env.XFERS_URL}/charges`, {
                headers: {
                    "X-XFERS-USER-API-KEY": process.env.XFERS_USER_API,
                ...
                params: {
                    ...
                    return_url: process.env.RETURN_URL+"/"+race_name,
                    notify_url: process.env.NOTIFY_URL,
                    cancel_url: process.env.CANCEL_URL+"/"+orderNum
                }
            });
            if(result) {
